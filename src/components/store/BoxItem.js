...
export default function BoxItem(props) {
  ...
  const deleteItem = () => {
    props.deleteItemFromBox(props.product.id)
  }
  return (
    <li className="list-group-item d-flex justify-content-between align-items-center px-0 py-2">
      <span>
        <span className="bg-dark rounded text-white mr-2" id="box_icon">
        ...
        <span className="overflow-auto">{product.name}</span>

      </span>
      <button className="btn btn-outline-danger" id="btn_delete_item" onClick={deleteItem}>x</button>
    </li>
  )
}
